...
- [ ] Run `scripts/backup-sparkle-key.sh --output "/absolute/offline/path/Ushot-Sparkle-key.backup.enc"` from an interactive Terminal, record the encrypted file's SHA-256, and confirm no plaintext key was created in the repository, shell history or output location
