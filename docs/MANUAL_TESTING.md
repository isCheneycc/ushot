...
This section contains two independent release gates. Public artifact identity, the `publish_update_feed=false` workflow-admission and recovery cases, and one local DMG installation smoke test block the explicitly labeled initial direct-download preview; the complete clean-standard-account first-install matrix additionally blocks a broader compatibility claim. Feed generation, update privacy, old-version → new-version replacement, authenticity/failure handling and active-work admission block `publish_update_feed=true`. The complete non-Developer-ID matrix has not been completed. Record the source/target versions, Git tags, asset hashes, macOS version, account type, network conditions, Screen Recording state and relevant privacy-safe logs for every run; do not infer a pass from a successful build or appcast generation. Even a completed matrix cannot waive Sparkle 2.9.5's missing public pre-install check of the extracted app's short/build versions: production self-update remains blocked until that client-side gap is closed.
...
- [ ] Before approval, confirm the protected `release` environment allows only the exact current tag. For 0.1.1 it must contain `v0.1.1`, not `v*` or `v0.1.0`; confirm the tombstoned candidate cannot enter the protected build-and-sign job
...
- [ ] For the first future production-feed activation, after a separate review binds seed admission to that exact version/build, confirm the repository appcast and copied fallback contain exactly one fixed Ushot channel and zero `item` elements. Require both the new marketing version and build to be strictly greater than the published direct-download baseline 0.1.1 build 2 even though the seed has no items. This zero-item seed may migrate into the embedded-notes format. Add an item or change the channel link in a disposable test branch and confirm signing fails even if the production URL returns 404
