...
- Published 0.1.2 (build 3) as the one-time manually installed hardened transition with `publish_update_feed=false`. Its runtime fork requires `SURequireExactUpdateVersionIdentity=true`, `SURequireEdDSAUpdateArchiveSignature=true` and framework capability marker `SUUpdateVersionIdentityHardeningVersion=1`, then rejects extracted short/build mismatch and archive EdDSA failure without a code-signing fallback. The immutable five-asset Release passed anonymous public download verification while the v1 feed remained absent.
- Protected the EdDSA private key and created an independently encrypted backup whose public-key derivation and decrypt/byte-compare recovery check passed; recovered-key disposable-update signing and client acceptance remain part of the first-feed matrix. Supply-chain checks and the privacy/network boundary are documented.
