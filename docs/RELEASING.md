...
The archive-version check above is a protected publication gate, while the reviewed runtime fork adds the separate post-extraction check and strict archive-EdDSA policy. The published 0.1.2 dependency remains historically pinned to `https://github.com/isCheneycc/Sparkle`, exact immutable tag `2.9.5-ushot.2`, revision `f4d8362bf9b6231596db3a0cc8812fdca8100961`; it predates the raw authenticated-XML hook. Fork tag `2.9.5-ushot.3` is retained only as the historical authenticated-XML-hook predecessor. Ushot 0.1.3 must instead pin the [published bounded-feed successor Release](https://github.com/isCheneycc/Sparkle/releases/tag/2.9.5-ushot.4), exact tag `2.9.5-ushot.4`, revision `3d81360ff115ffb80222c2723d72cb4cfa802774`, whose SwiftPM ZIP SHA-256 is `d8d36e5b5ee9e97b17babc1beeb26795b96558cafe5450130aafa5b169d5c829`; Release/CI builds must continue to refuse automatic package resolution. The runtime fork does not replace the publisher: appcasts and archives continue to be generated and signed with the official upstream Sparkle 2.9.5 tools. The fork Release is verified dependency evidence, not Ushot 0.1.3 publication or runtime evidence. Keep production self-update blocked until the built 0.1.3 framework/configuration and the complete 0.1.3 → 0.1.4 runtime matrix have recorded evidence; source review or a successful CI run alone cannot waive it.
...
4. Create a protected GitHub Environment named `release`, add required reviewers, and use exact tag deployment rules. Add only the tag currently approved for publication, such as `v0.1.3` for the second manual transition and later `v0.1.4` for the separately approved first feed; never use a `v*` wildcard. This general approval environment must contain no `SPARKLE_ED25519_PRIVATE_KEY` secret. A failed or superseded tag stays permanently absent from this allowlist because its immutable workflow source cannot be revoked from `main`.
...
The completed 0.1.2 run used `publish_update_feed=false`. It published five immutable direct-download assets, verified every remote byte, and downloaded all five again through the anonymous public boundary. It skipped Sparkle tool download, current-feed access, private-key signing, Pages artifact creation and `deploy-appcast`. Preserve that evidence exactly. Ushot 0.1.3 must follow the same false path and must not be described as published until its own protected run and anonymous verification succeed. Users must install both transitions manually as applicable. **Check for Updates…** remains visible and reports a feed failure at the v1 endpoint through the 0.1.3 manual stage.

Only after 0.1.3 is actually published, the operator-run encrypted-key recovery drill is recorded, and the complete clean-account 0.1.3 → 0.1.4 authenticated-XML/helper/replacement/tamper/exact-version/active-work matrix passes may an operator dispatch the first feed path:
...
Ushot 0.1.3 (build 4) has no published Release or immutable asset hashes yet. Do not append a second asset table, replace the 0.1.2 evidence or describe the candidate as downloadable until its separate `publish_update_feed=false` run succeeds and is anonymously verified.
...
Those checks admit only the direct-download Release. They preserve 0.1.2 as the published first manual transition and may admit 0.1.3 only as the second manual transition, never as a feed. Before publishing 0.1.3 with `publish_update_feed=false`, additionally verify the three required host booleans, exact `SUMaximumSignedAppcastContentLength=1048576`, all three framework markers, live hook ownership, pipeline duplicate/wrong-namespace/DTD/entity rejection, declared and streamed/chunked appcast oversize rejection, feed-cap exemption for persistent archives, and fail-closed missing/wrong configuration fixtures. Before the first `publish_update_feed=true` run for 0.1.4 (build 5), the separate 0.1.3 → 0.1.4 updater matrix must then verify:
