...
The GitHub distribution has no Developer ID signature or Apple notarization. Installing the published 0.1.2 transition, the upcoming manual-only 0.1.3 transition, or a later online update can cause macOS to request Screen Recording or Accessibility permission again. Ushot 0.1.3 is not yet published. Apple Developer Program membership is not required for this ad-hoc distribution.
...
Only when the user chooses **Check for Updates…** does Ushot make an update request. The published 0.1.1 client requests the legacy `https://ischeneycc.github.io/ushot/updates/appcast.xml`, which remains permanently unavailable and therefore cannot lead to an archive download. The published 0.1.2 transition and the not-yet-published 0.1.3 manual transition instead use the v1 boundary:
