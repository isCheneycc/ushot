...
> [!IMPORTANT]
> Ushot 0.1.11 (build 12) is the current public release and production update. Protected run [`32979927495`](https://github.com/isCheneycc/ushot/actions/runs/32979927495) published and anonymously verified its immutable five-asset GitHub Release, then deployed the independently authenticated v1 feed with `publish_update_feed=true`; supported installed versions can now reach 0.1.11 through **Check for Updates…**.
...
- **Accessibility is optional.** It refines smart region snapping from an application window to a stable control hierarchy. During capture, Tab selects the next parent level, Shift-Tab selects the previous child level, and Option-scroll remains available. Ordinary capture and window-level snapping continue to work when permission is absent, denied, or revoked.
...
For a first installation, download [Ushot-0.1.11-arm64.dmg](https://github.com/isCheneycc/ushot/releases/download/v0.1.11/Ushot-0.1.11-arm64.dmg) from the immutable [Ushot v0.1.11 Release](https://github.com/isCheneycc/ushot/releases/tag/v0.1.11) and install it manually. Public artifacts are intentionally ad-hoc signed, have no Developer ID signature or Apple notarization, and are not sandboxed.
...
The 0.1.1 client remains pinned to the legacy `/updates/appcast.xml` endpoint, which stays permanently unavailable (HTTP 404). Ushot 0.1.2 and later point at the v1 feed. Its first item was 0.1.4 (build 5), and its current item is 0.1.11 (build 12): supported installed versions can use **Check for Updates…** to download and install 0.1.11. The authenticated feed retains 0.1.10, 0.1.9, 0.1.6, and 0.1.5 as history; direct-download-only 0.1.7 and 0.1.8 were intentionally not inserted retroactively. A 0.1.1 or 0.1.2 install should install the current release manually from GitHub.
