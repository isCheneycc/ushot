...
> [!IMPORTANT]
> Ushot 0.1.7（build 8）是当前公开的直接下载版本。请从官方不可变 GitHub Release 手动安装。受保护运行 [`32689508333`](https://github.com/isCheneycc/ushot/actions/runs/32689508333) 以 `publish_update_feed=false` 发布了此版本；生产 v1 更新源仍停留在 0.1.6（build 7），因此**检查更新…**不会提供 0.1.7。
...
Ushot 0.1.7（build 8）是当前已发布的直接下载版本。受保护运行 [`32689508333`](https://github.com/isCheneycc/ushot/actions/runs/32689508333) 以 `publish_update_feed=false` 发布了其不可变五资产 GitHub Release，且没有扩展已签名的 v1 appcast。生产 v1 更新源仍停留在 0.1.6（build 7），因此 0.1.7 必须手动安装。直接从 `main` 构建的版本仍属于开发产物，而不是受支持的分发渠道。

请从不可变的 [Ushot v0.1.7 Release](https://github.com/isCheneycc/ushot/releases/tag/v0.1.7) 下载 [Ushot-0.1.7-arm64.dmg](https://github.com/isCheneycc/ushot/releases/download/v0.1.7/Ushot-0.1.7-arm64.dmg) 并手动安装。公开产物会刻意采用 ad-hoc 签名，不包含 Developer ID 签名或 Apple 公证，并且不会启用 App Sandbox。
