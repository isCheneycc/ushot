...
> [!IMPORTANT]
> Ushot 0.1.11（build 12）是当前公开版本与生产更新。受保护运行 [`32979927495`](https://github.com/isCheneycc/ushot/actions/runs/32979927495) 发布并匿名验证了其不可变五资产 GitHub Release，随后以 `publish_update_feed=true` 部署了经独立认证的 v1 更新源；受支持的已安装版本现在可通过**检查更新…**升级到 0.1.11。
...
Ushot 0.1.11（build 12）是当前已发布版本。受保护运行 [`32979927495`](https://github.com/isCheneycc/ushot/actions/runs/32979927495) 以 `publish_update_feed=true` 完成全部 11 个能力隔离的作业，发布并匿名验证了其不可变五资产 GitHub Release，同时部署了经独立认证的 v1 appcast。直接从 `main` 构建的版本仍属于开发产物，而不是受支持的分发渠道。

首次安装时，请从不可变的 [Ushot v0.1.11 Release](https://github.com/isCheneycc/ushot/releases/tag/v0.1.11) 下载 [Ushot-0.1.11-arm64.dmg](https://github.com/isCheneycc/ushot/releases/download/v0.1.11/Ushot-0.1.11-arm64.dmg) 并手动安装。公开产物会刻意采用 ad-hoc 签名，不包含 Developer ID 签名或 Apple 公证，并且不会启用 App Sandbox。
...
0.1.1 客户端仍固定使用旧 `/updates/appcast.xml` 端点，该端点将永久保持不可用（HTTP 404）。Ushot 0.1.2 及以后版本指向 v1 更新源。其首个版本是 0.1.4（build 5），当前版本是 0.1.11（build 12）：受支持的已安装版本可通过**检查更新…**下载并安装 0.1.11。认证后的更新源保留 0.1.10、0.1.9、0.1.6 与 0.1.5 作为历史；仅供直接下载的 0.1.7 和 0.1.8 不会被事后插入。0.1.1 或 0.1.2 安装应改为从 GitHub 手动安装当前发布版。
